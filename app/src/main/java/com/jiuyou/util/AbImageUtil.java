...
public class AbImageUtil {
	...
	public static String getDCTHashCode(Bitmap bitmap) {
		...
		int[][] pxMatrix = arrayToMatrix(pixels, width, height);
		double[][] doublePxMatrix = intToDoubleMatrix(pxMatrix);

		// 计算DCT,已经变成8*8了
		double[][] dtc = FDCT.fDctTransform(doublePxMatrix);

		// 计算平均值。
		double[] dctResult = matrixToArray(dtc);
		int avgPixel = average(dctResult);
	 ...
	 * @return int 平均值
	 */
	private static int average(double[] pixels) {
		float m = 0;
		for (int i = 0; i < pixels.length; ++i) {
			m += pixels[i];
		}
	 ...
	 * @return the int[][]
	 */
	private static int[][] arrayToMatrix(int[] m, int width, int height) {
	 ...
	 * @return the double[][]
	 */
	private static double[][] intToDoubleMatrix(int[][] input) {
