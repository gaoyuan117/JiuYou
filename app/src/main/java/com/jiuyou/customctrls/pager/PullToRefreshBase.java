...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    // 是否拉取到数据
    protected boolean isPulledData = false;
    ...
    public void setOpenRecommandMode(boolean isOpenRecommandMode) {
        ...
        mHeaderLayout.setOpenRecommandMode(isOpenRecommandMode);
        mFooterLayout.setOpenRecommandMode(isOpenRecommandMode);
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
                    Log.d("x_as", "isRefreshing");
                    return true;
                }

                if (Math.abs((int) mLastMotionX - (int) mInitialMotionX) > Math.abs((int) mLastMotionY - (int) mInitialMotionY)
                        && Math.abs((int) mLastMotionX - (int) mInitialMotionX) > mTouchSlop) {
//                    Log.d("x_as", "continue " + "-(int)mLastMotionY-" + ((int) mLastMotionY) + "=mInitialMotionY=" + ((int) mInitialMotionY)
//                            + "-(int)mLastMotionX-" + ((int) mLastMotionX) + "-(int)mInitialMotionY-" + ((int) mInitialMotionX) + "-mTouchSlop-" + mTouchSlop);

                    return false;
                } else {
//                    Log.d("x_as", "no capture " + "-(int)mLastMotionY-" + ((int) mLastMotionY) + "=mInitialMotionY=" + ((int) mInitialMotionY)
//                            + "-(int)mLastMotionX-" + ((int) mLastMotionX) + "-(int)mInitialMotionY-" + ((int) mInitialMotionX) + "-mTouchSlop-" + mTouchSlop);
    ...
    public final void onRefreshComplete(int dataSize) {

        if (!NetStatusUtils.isNetWorkEnableWithoutToast()) {
            onRefreshCompleteNoNet();
            return;
        }


        this.isPulledData = dataSize > 0;
        this.dataSize = dataSize;
        if (isPullingData) {
            setState(State.LOADING_FINISHED);
        }
    }

    public final void onRefreshCompleteNoNet() {

        setState(State.NO_NET);

    }

    // 下拉刷新：0    上拉刷新：1  中间状态:-1
    public static final int REFRESH_MODE_PULL_START = 0;
    public static final int REFRESH_MODE_PULL_END = 1;
    public static final int REFRESH_MODE_IDLE = -1;
    protected int curRefreshingMode = REFRESH_MODE_IDLE;

    // 是否开启搜索栏模式
    public boolean isOpenSearchMode = false;

    public int getCurRefreshingMode() {
        return curRefreshingMode;
    }

    public int searchBarHeight = (int) getResources().getDimension(R.dimen.pager_head_search_bar_height);
    protected boolean isShowingSearch = false;


    public void showRefreshingBarProgress(int height) {
        isShowingSearch = true;


        smoothScrollTo(height, 200);
    }


    public void autoRefreshing() {
        curRefreshingMode = REFRESH_MODE_PULL_START;
//        Log.d("asdsddddd", " 0");
        setState(State.REFRESHING, true);
    }

    public void autoRefreshing2() {
        curRefreshingMode = REFRESH_MODE_PULL_END;
        setState(State.REFRESHING, true);
    }
    ...
    public final boolean onTouchEvent(MotionEvent event) {
        ...
        // 当刷新成功的文本提示正显示时，触摸后将头部状态重置
        if (isShowingTextTip) mHeaderLayout.onTextTipReset();
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    if (mState == State.RELEASE_TO_REFRESH
                        ...
                        autoRefreshing();
                        return true;
                    } else if (isOpenSearchMode && mLastMotionY - mInitialMotionY > searchBarHeight / 2) {
                        // 显示刷新栏高度
...
//                        Log.d("asdsddddd", "1");
                    ...
                    if (isRefreshing()) {
                        curRefreshingMode = REFRESH_MODE_IDLE;
                        smoothScrollTo(0);
//                        Log.d("asdsddddd", "3");
                        return true;
                    }
...
//            Log.d(LOG_TAG, "State: " + mState.name());
    ...
    protected LoadingLayoutBase createLoadingLayout(Context context, Mode mode,
                                                    TypedArray attrs) {
        LoadingLayoutBase layout = mLoadingAnimationStyle.createLoadingLayout(
    ...
    // 头部需要重置
    protected boolean isNeedHeadRealReset = true;
    ...
    protected void onReset() {
        ...
        // Always reset both layouts, just in case...
        mHeaderLayout.reset();
        mFooterLayout.reset(isPulledData, dataSize);
        if (isNeedHeadRealReset) {
//            Log.d("pulltorefresh","isNeedHeadRealReset");
            smoothScrollTo(0, getPullToRefreshScrollDuration(),
                    0, new OnSmoothScrollFinishedListener() {
                        @Override
                        public void onSmoothScrollFinished() {
                            mHeaderLayout.changeViewType(0, 0);
                        }
                    });
//            smoothScrollTo(0);
        } else {
//            Log.d("pulltorefresh","isNeedHeadRealReset no");
            int height = mHeaderLayout.getPulledDataTextCtrlHeight();
//            Log.d("sadasda", "height-" + height);
...
//			addViewInternal(mFooterLayout, lp);
    ...
    // 正在拉取数据
    protected boolean isPullingData = false;

    protected void callRefreshListener() {

//        if(1==1){onRefreshCompleteNoNet();return;}
        ...
        if (null != mOnRefreshListener) {
            ...
            if (getCurRefreshingMode() == PullToRefreshBase.REFRESH_MODE_PULL_START) {
                mOnRefreshListener2.onPullDownToRefresh(this);
            } else if (getCurRefreshingMode() == PullToRefreshBase.REFRESH_MODE_PULL_END) {
                mOnRefreshListener2.onPullUpToRefresh(this);
            }
        }
    }

    public boolean isShowingTextTip = false;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // recycle them...
        // listview装载head，foot 用于refreshing
        ...
        switch (mCurrentMode) {
            ...
            default:
                ...
                newScrollValue = Math.round(Math.min(initialMotionValue
                        - lastMotionValue, 0)
                        / FRICTION);
                if (isOpenSearchMode && isShowingSearch) {
                    newScrollValue = newScrollValue - searchBarHeight;
                }
                // 获取顶部view的height
...
//                isShowingSearch = false;
    ...
    public static enum AnimationStyle {
        ...
        LoadingLayoutBase createLoadingLayout(Context context, Mode mode,
                                              Orientation scrollDirection, TypedArray attrs) {
            switch (this) {
                case ROTATE:
                default:

                    RotateLoadingLayout rll = new RotateLoadingLayout(context, mode, scrollDirection,
                            attrs);

//                    rll.setShowSearchBar(isOpenSearchMode);
